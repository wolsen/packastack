...
def test_get_current_cycle_list_finds_second_item(temp_releases_repo):
    """Test list where first item doesn't match but second has development."""
    series_data = [
    ...
    cycle = get_current_cycle(temp_releases_repo)
    assert cycle == "dalmatian"


def test_get_current_cycle_list_development_with_name(temp_releases_repo):
    """Test that returns development cycle name when present."""
    series_data = [
        {"name": "gazpacho", "status": "development"},
    ...
    cycle = get_current_cycle(temp_releases_repo)
    assert cycle == "gazpacho"


def test_get_current_cycle_no_development_list_format(temp_releases_repo):
    """Test error when no development cycle found in list format."""
    series_data = [
        {"name": "caracal", "status": "released"},
        {"name": "dalmatian", "status": "released"},
    ]
    ...
    with pytest.raises(ImporterError, match="No development cycle found"):
        get_current_cycle(temp_releases_repo)


def test_get_current_cycle_list_with_non_development_dict(temp_releases_repo):
    """Test list with non-dict items mixed with development dict."""
    series_data = [
        "invalid_item",  # Mixed with non-dict BEFORE development
        {"name": "caracal", "status": "released"},
        {"name": "dalmatian", "status": "development"},
    ]
...
def test_get_current_cycle_list_with_missing_name(temp_releases_repo):
    """Test list with development entry missing name field."""
    series_data = [
        {"status": "development"},  # No name field
        {"name": "caracal", "status": "released"},
    ]
...
def test_get_current_cycle_development_with_empty_name(temp_releases_repo):
    """Test development entry with empty name field (falsy value)."""
    series_data = [
        {"name": "", "status": "development"},  # Empty name (falsy)
        {"name": "caracal", "status": "development"},
    ]
    ...
    # Should skip the empty name and find caracal
    cycle = get_current_cycle(temp_releases_repo)
    assert cycle == "caracal"


def test_get_current_cycle_non_list_format(temp_releases_repo):
    """Test error when series_status.yaml is not list format."""
    series_data = {  # Dict instead of list
        "dalmatian": {"status": "development"}
    }
...
def test_get_previous_cycle_success_list_format(temp_releases_repo):
    """Test getting previous released cycle from list format YAML."""
    # Reverse chronological: first is development, second is previous
    series_data = [
        {"name": "dalmatian", "status": "development"},
        {"name": "caracal", "status": "released"},
    ...
    cycle = get_previous_cycle(temp_releases_repo)
    assert cycle == "caracal"


def test_get_previous_cycle_released_with_name(temp_releases_repo):
    """Test that returns released cycle name when present."""
    series_data = [
        {"name": "dalmatian", "status": "development"},
        {"name": "epoxy", "status": "released"},
    ]
...
def test_get_previous_cycle_none_found_list_format(temp_releases_repo):
    """Test when no previous cycle exists in list format."""
    series_data = [
        {"name": "dalmatian", "status": "development"},
    ]
...
def test_get_previous_cycle_list_with_non_dict_items(temp_releases_repo):
    """Test list with non-dict items and released dict items."""
    # Reverse chronological: first is development, second is non-dict
    ...
    with pytest.raises(AssertionError):
        get_previous_cycle(temp_releases_repo)


def test_get_previous_cycle_list_with_missing_name(temp_releases_repo):
    """Test list with released entry missing name field."""
    # Reverse chronological: development first, then released entry missing name
    series_data = [
        {"name": "dalmatian", "status": "development"},
        {"status": "released"},  # No name field
        {"name": "caracal", "status": "released"},
    ]
    ...
    cycle = get_previous_cycle(temp_releases_repo)
    # The second entry has no name, so we return None (no fallback).
    assert cycle is None


def test_get_previous_cycle_released_with_empty_name(temp_releases_repo):
    """Test released entry with empty name field (falsy value)."""
    series_data = [
        {"name": "dalmatian", "status": "development"},
        {"name": "", "status": "released"},  # Empty name (falsy)
        {"name": "caracal", "status": "released"},
    ]
    ...
    # The second entry has an empty name; no fallback is performed.
    cycle = get_previous_cycle(temp_releases_repo)
    assert cycle is None


def test_get_previous_cycle_non_list_format(temp_releases_repo):
    """Test error when series_status.yaml is not list format."""
    series_data = {  # Dict instead of list
        "dalmatian": {"status": "development"}
    }
...
"""
    ...
    key_id, key_data = get_signing_key(temp_releases_repo)
...
def test_get_deliverable_info_success(temp_releases_repo):
    ...
    info = get_deliverable_info(temp_releases_repo, "dalmatian", "nova")
...
def test_get_deliverable_info_parse_error(temp_releases_repo):
    ...
    with pytest.raises(ImporterError, match="Failed to parse deliverable file"):
        get_deliverable_info(temp_releases_repo, "dalmatian", "nova")
